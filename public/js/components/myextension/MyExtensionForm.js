	...
	methods: {
		sendForm() {
			if (this.$refs.form) // We can only send when $refs is ready
				this.$refs.form
					.post('extensions/FHC-Core-Developer/components/MyExtensionAPI/saveExampledata', this.formData)
					.then(result => {
		...
		prefillFormdata(exampledata_id){
			CoreRESTClient
				.get('/extensions/FHC-Core-Developer/MyExtension/getExampledata', { exampledata_id: exampledata_id })
