	...
	methods: {
		loadAutocompleteSuggestions(event) {
			if (event.query || !this.autocompleteSuggestions.length) {
				...
				this.$fhcApi
					.get(
						'extensions/FHC-Core-Developer/components/form/autocompleteSuggestions/' + encodeURIComponent(event.query),
						null,
						{
		...
		sendForm() {
			...
			if (this.$refs.form) {
				...
				this.$refs.form
					.post(
						'extensions/FHC-Core-Developer/components/form/full',
						formData
					)
					.then(result => {
